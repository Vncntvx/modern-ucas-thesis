...
2022年修订的《中国科学院大学 研究生学位论文撰写规范和指导意见》（以下简称《指导意见》）从2023年冬季批次开始实施。这一份基于 Typst 的社区模板（非学校官方出品），用于辅助按《指导意见》撰写硕士、博士学位论文，并兼顾本科生毕业论文（设计）的常用结构。使用时，您只需在相应章节填写研究内容，并按第2章说明配置论文元信息即可；具体格式以所在培养单位的最新要求为准，提交前请自行核对。
...
输出格式以 PDF 为主，较新版本已支持导出 HTML（公式经 MathML 转换）。本模板在 `typst.toml` 中声明 `compiler = "0.15.0"`，建议使用 0.15.0 及以上版本的 Typst 编译本文档。
...
从 LaTeX 迁移到本模板的成本有限：导言区中的文档类与宏包选项大体对应 `documentclass(...)` 的参数，即论文元信息（题目、作者、导师、专业等）与 `twoside`、`anonymous` 等模式开关；`\input{chapter}` 式的章节组织对应主文件中的章节划分或 `#include`。作者随后按学位论文的物理顺序调用 `cover`、`decl-page`、`abstract`、`mainmatter` 等函数即可，正文写作以 Markdown 式标记为主，无需掌握 Typst 的全部语法。

== 系统要求<sec:system>

前文所述的增量编译与实时预览，需要具体的运行环境支撑。Typst 官方支持 Windows、macOS 与 Linux，使用方式分为本地与网页两类，学位论文撰写推荐本地方案。本地方案以 Typst CLI 为核心，并搭配 VS Code、Neovim、Emacs 等编辑器的官方或社区扩展，以获得语法高亮、自动补全、定义跳转与侧边预览等语言服务；安装包请通过官方发布页或系统包管理器获取，避免非官方渠道构建带来的兼容问题。

网页版 #link("https://typst.app/")[Typst Web App] 是无需本地安装的替代途径：在浏览器中即可在线编辑与实时预览，并支持多人协作，适合正式撰写前的快速试用；使用本模板时需手动上传 `fonts/` 目录下的字体文件，否则相应字体无法正常显示。

在本地方案中，CLI 提供 `compile`（单次编译）与 `watch`（监听变更并自动重编译）两种常用模式：撰写阶段通常使用 `watch`，并以 `--font-path` 指定本模板的 `fonts/` 目录，具体命令与项目组织见第2章。各环境的界面、编译方式、预览方案与语言服务等方面的对比见 @tbl:Typst_intro。
...
#auto-table(
  ...
  columns: 6,
  align: center,
  header: ([名称], [界面], [编译方式], [预览方案], [增量编译], [语言服务]),
  label: <Typst_intro>,
  [VS Code], [VS Code], [本地 CLI], [扩展 Webview], [watch 下是], [良好],
  [Neovim], [Neovim], [本地 CLI], [插件/Webview], [watch 下是], [良好],
  [Emacs], [Emacs], [本地 CLI], [插件/Webview], [watch 下是], [良好],
  [CLI], [任意编辑器], [本地原生], [外部 PDF 阅读器], [watch 下是], [依赖编辑器插件],
  [Web App], [浏览器], [云端/WASM], [内置实时预览], [是], [优秀],
)
