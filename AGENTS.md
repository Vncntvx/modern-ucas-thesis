...
Returned functions: `doc` (A4, Word margins), `cover`, `notice` (填表说明), `outline-page` (outline-form TOC linked to body headings), `mainmatter` (heading numbering `1.` / `1.1.` / `1.1.2`, 2em first-line indent, footer `第X页，共Y页`), `bilingual-bibliography` (`page-decoration: none`), plus figure/table helpers. `info` must fill supervisors as **exactly one** of `supervisors-full` or `supervisors-split` (both or neither panics; no `supervisor-form`; `supervisors-split` requires a non-empty `name`). Validation runs in `proposalclass` at assembly time, not deferred to `cover`. Compile with `typst compile template/proposal.typ --root . --font-path fonts`.
...
- Outline indent: the `level` passed to the `outline(indent:)` callback is 0-based (unlike `outline.entry.level`, which is 1-based; undocumented, measured 0/12/24pt hitting "flush/one-char/two-char" exactly). The `slice(0, level+1)` logic in `outline-page.typ` is correct. **Do not** "fix" it as an off-by-one. (2026-09 re-verification: PyMuPDF span origins x = 89.86 / 101.86 / 113.86pt = flush / +12pt / +24pt exactly.)
- Outline fill/page policy (intentional, thesis + proposal): dots and `entry.page()` are rendered as a **sibling** `text(font: ("Times New Roman",), size: 字号.小四)` immediately after the entry-line `text` that carries the heading size (not nested inside it — nesting shifts multi-line level-1 entry baselines by ~1.5pt). Level-1 page numbers are therefore **12pt Times**, not 14pt with the chapter title. Leader default is `repeat([.], gap: 0.12em)` (denser than Typst's official `0.15em`), wrapped as `box(width: 1fr, inset: (x: .25em), fill)`. **Do not** nest aux inside the title `text` without re-baselining TOC spans, and **do not** "restore" same-size page numbers unless docs + this conclusion are updated together. Proposal keeps `outline-gap: 1.3em` and depth 2; thesis keeps `gap: .3em` and depth 3 — do not unify.
...
- **Bachelor styles only**: changes must not affect graduate (master/doctor) output **except** the intentionally shared outline fill/page policy above (thesis + proposal TOC aux is Times 小四 + dense leaders). Every bachelor-vs-graduate difference is gated on `doctype`, with the graduate branch preserving the original code path verbatim (`layouts/doc.typ`, `layouts/mainmatter.typ`, `layouts/appendix.typ`, `layouts/preface.typ`, `utils/page-foreground.typ`, `utils/bilingual-bibliography.typ`, `pages/acknowledgement.typ`, `pages/backmatter.typ`, `pages/list-of-figures-and-tables.typ`, `pages/notation.typ`; each takes an additive `doctype: "doctor"` parameter that `lib.typ` fills in). `pages/outline-page.typ` is frozen **except** for the documented TOC aux policy (nested Times 小四 fill/page, 0.12em leader) — any other graduate layout change there is still forbidden. **Verify after every change**: materialize the pre-change tree, compile `template/thesis.typ` in both trees, and diff every page's text spans (position/size/font/text) plus vector drawings with PyMuPDF — expect zero differing pages **outside** TOC outline-entry aux spans when only the documented aux policy is involved; any other page/span diff is a regression.
